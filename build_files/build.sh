...
## Shell completions, generated by the binary that was just installed, so they
## cannot drift from it the way a checked-in copy would.
##
## The three directories belong to the "filesystem" package rather than to zsh
## or fish, so they are present on every base whether or not those shells are
## installed - which means the completions are already in place if either is
## added to rpm_packages later, and nothing here has to check first.
##
## herdr also emits powershell and elvish. Neither is generated, because
## neither has a system-wide completion directory on Linux to put it in.
